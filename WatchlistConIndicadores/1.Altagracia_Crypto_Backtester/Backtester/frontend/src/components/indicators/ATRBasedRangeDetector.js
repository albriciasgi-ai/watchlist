...
class ATRBasedRangeDetector extends IndicatorBase {
  ...
  /**
   * 🎯 NUEVO: Obtener rangos filtrados para rendering en backtesting
   * @param {number} maxTimestamp - Máximo timestamp a mostrar (currentTime en backtesting)
   */
  getRangesToRender(maxTimestamp = null) {
    if (!maxTimestamp) {
      // Modo normal: devolver todos los rangos
      return this.detectedRanges;
    }

    // Modo backtesting: filtrar rangos que no deberían mostrarse
    return this.detectedRanges.filter(range => {
      // Solo mostrar rangos que han comenzado antes de maxTimestamp
      // y no mostrar rangos que comienzan después de maxTimestamp
      return range.startTimestamp <= maxTimestamp;
    }).map(range => {
      // Si el rango se extiende más allá de maxTimestamp, limitarlo
      if (range.endTimestamp > maxTimestamp) {
        return {
          ...range,
          endTimestamp: maxTimestamp,
          isLimited: true  // Flag para indicar que está limitado
        };
      }
      return range;
    });
