...
class ZoneVisualizerIndicator extends IndicatorBase {
  constructor(symbol, interval, days = 30) {
    ...
    this.config = {
      ...
      colors: {
        ...
        // Strategy Builder colores (verde teal para OrderFlow)
        sb_zone: { fill: 'rgba(0, 137, 123, 0.18)', border: '#00897B' },
        sb_win: { fill: 'rgba(0, 180, 100, 0.25)', border: '#00B464' },
        sb_loss: { fill: 'rgba(210, 50, 50, 0.25)', border: '#D23232' },
        sb_open: { fill: 'rgba(0, 137, 123, 0.12)', border: '#00897B' },
  ...
  setStrategyZones(zones) {
    ...
    this._mergeZones();
    this._renderLogged = false; // Reset para loguear con cada nuevo backtest
    this._noRenderLogged = false;
  ...
  renderOverlay(ctx, bounds, visibleCandles, allCandles, priceContext = null) {
    ...
    // Log una vez por sesion
    if (!this._renderLogged) {
      console.log(`[${this.symbol}] ZoneVisualizer: ${this.zones.length} zonas, hasTimeToX=${!!priceContext.timeToX}`);
  ...
  _renderZoneOverlay(ctx, zone, visibleCandles, allCandles, bounds, priceContext) {
    ...
    // Colores de consolidacion: gris para sin trade, cyan para PENDING, azul VP, verde SB, azul gris V2
    ...
    for (let tIdx = 0; tIdx < tradesToDraw.length; tIdx++) {
      ...
      let entryOffScreen = false;

      if (timeToX) {
        const tx1 = timeToX(trade.entry_timestamp);
        if (tx1 !== null && tx1 !== undefined) {
          if (tx1 < boundsX) entryOffScreen = true;
          xTradeStart = Math.max(boundsX, tx1);
        }
        ...
        // Si el trade esta cerrado pero el entry esta fuera del viewport,
        // limitar el ancho visible del rectangulo para no cubrir todo el chart
        if (entryOffScreen && !isOpen && trade.trade_close_timestamp) {
          // Usar close_ts como referencia y extender solo un poco hacia la izquierda
          const barsHeld = trade.bars_held || 10;
          const estimatedBarWidth = boundsWidth / (visibleCandles ? visibleCandles.length : 200);
          const maxVisualWidth = Math.min(barsHeld * estimatedBarWidth, boundsWidth * 0.4);
          xTradeStart = Math.max(boundsX, xTradeEnd - maxVisualWidth);
        }
      }
