/**
 * CandleCache.js - Sistema de cache persistente para velas historicas
 *
...
class CandleCache {
  // TTL para velas cerradas (24 horas - las velas historicas no cambian)
  static CLOSED_CANDLE_TTL = 24 * 60 * 60 * 1000;

  // 🚀 LRU: Maximo de entradas en memoria (el resto queda en IndexedDB)
  static MAX_MEMORY_ENTRIES = 4;

  // Minimo de velas esperadas por dia segun timeframe
  ...
  // Cache en memoria para acceso rapido (LRU order: mas reciente al final)
  static memoryCache = new Map();

  // Lista de claves en orden LRU (mas antiguo primero, mas reciente al final)
  static lruOrder = [];
  ...
  static async set(symbol, interval, candles) {
    ...
    // Remover vela en progreso (la ultima si esta marcada como in_progress)
    ...
    // 🚀 LRU: Guardar en memoria con limite
  ...
  static async getValidated(symbol, interval, days) {
    ...
    // Detectar gaps en el cache - si hay gaps, limpiar y forzar recarga
