...
class DoubleTopBottomIndicator extends IndicatorBase {
  constructor(symbol, interval, days = 90) {
    ...
    console.log(`[${this.symbol}] 🔔 Double Top/Bottom alert system initialized with multi-level support`);
  }
  ...
  loadConfig() {
    ...
    // SIEMPRE actualizar maxBreakoutPercent según el timeframe actual
    ...
    // FORZAR deshabilitación del filtro de volumen para evitar rechazos incorrectos
    if (config.doubleTopBottom.volumeFilter) {
      config.doubleTopBottom.volumeFilter.enabled = false;
    }

    // IMPORTANTE: Forzar valores actualizados de filtros para solucionar problema de detección
    if (!config.filters) {
      config.filters = {};
    }
    config.filters.minConfidence = 20;  // Forzar 20% en lugar del valor guardado
    config.filters.requireBothRejections = false;  // Asegurar que esté deshabilitado

    console.log(`[${this.symbol}] DTB Config loaded - maxBreakoutPercent: ${config.doubleTopBottom.maxBreakoutPercent}% for interval ${this.interval}, minConfidence: ${config.filters.minConfidence}%, volumeFilter: disabled, requireBothRejections: ${config.filters.requireBothRejections}`);
  ...
  async fetchData() {
    // Recargar la configuración para asegurar que maxBreakoutPercent sea dinámico según el timeframe
    this.config = this.loadConfig();
    ...
    try {
      console.log(`[${this.symbol}] 🔍 DTB FETCH START ====================================`);
      console.log(`[${this.symbol}] Interval: ${this.interval}, Days: ${this.days}`);
      console.log(`[${this.symbol}] Config being sent to backend:`);
      console.log(`[${this.symbol}]   - maxBreakoutPercent: ${this.config.doubleTopBottom?.maxBreakoutPercent}%`);
      console.log(`[${this.symbol}]   - minConfidence: ${this.config.filters?.minConfidence}%`);
      console.log(`[${this.symbol}]   - requireBothRejections: ${this.config.filters?.requireBothRejections}`);
      console.log(`[${this.symbol}]   - volumeFilter.enabled: ${this.config.doubleTopBottom?.volumeFilter?.enabled}`);
      console.log(`[${this.symbol}]   - lookbackCandles: ${this.config.doubleTopBottom?.lookbackCandles}`);
      ...
      console.log(`[${this.symbol}] Full request payload:`, JSON.stringify(requestPayload, null, 2));
      ...
      console.log(`[${this.symbol}] 📥 DTB RESPONSE ====================================`);
      console.log(`[${this.symbol}] Success: ${result.success}`);
      console.log(`[${this.symbol}] Duration: ${duration}s`);
      console.log(`[${this.symbol}] Patterns received: ${result.patterns ? result.patterns.length : 0}`);

      if (result.debug_info) {
        console.log(`[${this.symbol}] Debug info from backend:`, result.debug_info);
      }

      if (result.success && result.patterns) {
        this.patterns = result.patterns;
        console.log(`[${this.symbol}] ✅ Double Top/Bottom: ${this.patterns.length} patterns stored`);

        if (this.patterns.length > 0) {
          console.log(`[${this.symbol}] First pattern confidence: ${this.patterns[0].confidence}%`);
          console.log(`[${this.symbol}] Pattern types: ${this.patterns.map(p => p.type).join(', ')}`);
        }

        if (this.config.debugMode || this.patterns.length === 0) {
          console.log(`[${this.symbol}] All patterns:`, this.patterns);
        }
      } else {
        console.error(`[${this.symbol}] ❌ Double Top/Bottom detection failed:`, result.error);
        if (result.details) {
          console.error(`[${this.symbol}] Error details:`, result.details);
        }
        this.patterns = [];
      }
  ...
  loadAlertHistory() {
    try {
      ...
      if (saved) {
        ...
        console.log(`[${this.symbol}] Loaded ${this.alertHistory.length} alerts from history`);
      }
    } catch (e) {
      console.error(`[${this.symbol}] Failed to load alert history:`, e);
      this.alertHistory = [];
    }
  }
  ...
  saveAlertHistory() {
    try {
      ...
      console.error(`[${this.symbol}] Failed to save alert history:`, e);
    }
  }
  ...
  clearAlertHistory() {
    this.alertHistory = [];
    this.saveAlertHistory();
    console.log(`[${this.symbol}] Alert history cleared`);
  }
  ...
  checkVWAPAlignment(pattern) {
    ...
    if (!this.indicatorManager) {
      console.warn(`[${this.symbol}] IndicatorManager not available for VWAP filter`);
    ...
    if (!vwapIndicator || !vwapIndicator.enabled) {
      console.warn(`[${this.symbol}] VWAP filter enabled but VWAP indicator not active`);
    ...
    if (!deviations) {
      console.warn(`[${this.symbol}] VWAP indicator doesn't provide deviations`);
      return true;
    }
    ...
    if (direction === 'LONG') {
      ...
      if (requireDev2 && deviations.lower2) {
        ...
        if (near2) {
          console.log(`[${this.symbol}] ✅ VWAP aligned: LONG pattern near -2σ (${dev2.toFixed(2)})`);
          aligned = true;
        }
      }
      ...
      if (requireDev3 && deviations.lower3) {
        ...
        if (near3) {
          console.log(`[${this.symbol}] ✅ VWAP aligned: LONG pattern near -3σ (${dev3.toFixed(2)})`);
          aligned = true;
        }
      }
    ...
    if (direction === 'SHORT') {
      ...
      if (requireDev2 && deviations.upper2) {
        ...
        const near2 = Math.abs(patternPrice - dev2) / dev2 <= tolerance;
        if (near2) {
          console.log(`[${this.symbol}] ✅ VWAP aligned: SHORT pattern near +2σ (${dev2.toFixed(2)})`);
          aligned = true;
        }
      }
      ...
      if (requireDev3 && deviations.upper3) {
        ...
        const near3 = Math.abs(patternPrice - dev3) / dev3 <= tolerance;
        if (near3) {
          console.log(`[${this.symbol}] ✅ VWAP aligned: SHORT pattern near +3σ (${dev3.toFixed(2)})`);
          aligned = true;
        }
      }
  ...
  shouldSendAlert(pattern) {
    ...
    if (mode === 'smart') {
      ...
      if (pattern.entrySignal?.has_momentum) {
        ...
        if (level === null) {
          console.log(`[${this.symbol}] Pattern rejected: Confidence too low (${pattern.confidence.toFixed(1)}%)`);
          return false; // Confianza muy baja
        }
      } else {
        // Sin momentum → requiere alta confianza (70%+)
        if (pattern.confidence < 70) {
          console.log(`[${this.symbol}] Pattern rejected: No momentum and low confidence (${pattern.confidence.toFixed(1)}%)`);
          return false;
        }
      }
    }
    ...
    if (this.config.alertSettings.vwapFilter.enabled) {
      if (!this.checkVWAPAlignment(pattern)) {
        console.log(`[${this.symbol}] Pattern rejected: VWAP filter not met`);
    ...
    if (level && !this.checkCooldown(pattern, level)) {
      ...
      console.log(`[${this.symbol}] Pattern rejected: Cooldown active (${cooldownSeconds}s for ${level} level)`);
      return false;
    }
  ...
  async checkAndSendAlerts() {
    ...
    if (this.alertSystemStartTime === null) {
      ...
      console.log(`Mode: ${this.config.alertSettings.mode.toUpperCase()}`);
    ...
    console.log(`VWAP Filter: ${this.config.alertSettings.vwapFilter.enabled ? 'ENABLED' : 'DISABLED'}`);
    ...
    for (const pattern of this.patterns) {
      ...
      console.log(`  ✅ NEW: ${levelEmoji} ${pattern.type} at $${pattern.levelPrice.toFixed(2)} (${patternDate}) - Level: ${level?.toUpperCase() || 'N/A'}`);
    ...
    console.log(`  ⏭️  Failed validation: ${skipReasons.failedValidation}`);
    ...
    for (const pattern of newPatterns) {
      ...
      console.log(`     ${levelEmoji} Level: ${level?.toUpperCase() || 'N/A'}`);
    ...
    console.log(`  📋 History size: ${this.alertHistory.length}`);
    console.log(`${'='.repeat(80)}\n`);
  }
  ...
  showAlertPopup(pattern, level = null) {
    ...
    console.log(`%c[${this.symbol}] 🚨 DBT ALERT SENT ${levelText}`, `background: ${bgColor}; color: white; font-weight: bold; padding: 4px;`);
