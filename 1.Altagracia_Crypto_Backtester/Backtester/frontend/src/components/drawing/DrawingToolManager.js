...
class DrawingToolManager {
  ...
  handleMouseDown(x, y, scaleConverter, tool) {
    ...
    if (this.currentTool === 'tpsl') {
      const box = new TPSLBox(price, time, 'long');
      this.addShape(box);
      this.saveToHistory();
    ...
    if (this.currentTool === 'tpsl-short') {
      const box = new TPSLBox(price, time, 'short');
      this.addShape(box);
      this.saveToHistory();
      // NO cambiar tool, permitir dibujar múltiples cajas
      return true;
  ...
  isInteracting() {
    return this.selectedShape && (this.selectedShape.isDragging || this.selectedShape.isResizing);
  }
