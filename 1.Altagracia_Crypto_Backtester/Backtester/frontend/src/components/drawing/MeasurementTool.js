...
class MeasurementTool {
  constructor() {
    ...
    this.finishedAt = null; // Timestamp de cuando se completó
  }
  ...
  handleMouseDown(x, y, scaleConverter) {
    if (!this._isMeasuring) {
      ...
      this._isMeasuring = false;
      this.endPoint = { x, y };
      this.finishedAt = Date.now(); // Marcar cuando terminó
    }
  ...
  handleMouseMove(x, y) {
    ...
    if (!this._isMeasuring || !this.startPoint) return false;
  ...
  render(ctx, scaleConverter) {
    ...
    // Auto-limpiar si pasaron 2 segundos desde que terminó
    if (this.finishedAt && !this._isMeasuring) {
      const elapsed = Date.now() - this.finishedAt;
      if (elapsed > 2000) {
        this.clear();
        return;
      }
  ...
  renderInfoBox(ctx, m) {
    ...
    const instruction = this._isMeasuring
      ? 'Click rueda nuevamente para finalizar'
      : 'Se auto-limpiará en 2 seg (Esc para cancelar)';
    ctx.fillText(instruction, boxX + padding, boxY + boxHeight - 6);
  }
}
