...
def _get_detection_fingerprint(config_overrides: Dict, symbol: str,
    ...
    # IMPORTANTE: Incluir candles_count para invalidar cache cuando el array
    # de velas tiene tamano diferente (ej: cache incremental agrego velas).
    # Sin esto, los indices (detection_candle_idx) apuntan a velas incorrectas.
...
def save_zone_cache(zones: List[Dict], config_overrides: Dict,
    ...
    fingerprint = _get_detection_fingerprint(config_overrides, symbol, interval, days, candles_count)
...
def load_zone_cache(config_overrides: Dict, symbol: str,
                    interval: str, days: int,
                    candles_count: int = 0) -> Optional[Dict]:
    """
