...
class RealtimePatternService:
    ...
    async def _process_dbt_pattern(self, symbol: str, interval: str, pattern,
        ...
        # IMPORTANT: Use current time for rate limit check, not pattern timestamp
        # This is because DTB patterns are detected when the CONFIRMATION happens (current candle close),
        # but the pattern's timestamp is from the second extreme which could be hours old
        detection_time_ms = int(time.time() * 1000)
        can_send, rate_limit_reason = self._check_rate_limit(
            symbol, interval, 'doubleTopBottom', detection_time_ms, config
        )
        if not can_send:
