...
class SwingDetector:
    ...
    def detect(
        ...
        for i in range(start_idx, end_idx):
            ...
            # NOTA: Si no hay zonas configuradas, se permite la señal (zonas son opcionales)
            zone_match = None
            if price_zones:
                ...
                # Solo filtrar si hay zonas configuradas Y la señal no está en ninguna
                if not zone_match:
                    logger.debug(f"[{self.name}] {symbol}: Swing rejected - not in any price zone")
                    continue
