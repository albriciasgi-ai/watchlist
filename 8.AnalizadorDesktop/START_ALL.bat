...
echo ============================================================
echo   ANALIZADOR DESKTOP - INICIO AUTOMATICO
echo ============================================================
echo.

REM Verificar si el backend ya esta corriendo
echo [1/3] Verificando si el backend esta corriendo en puerto 10000...
...
echo [2/3] Iniciando backend del Analizador...
start "Analizador Backend" cmd /k "cd /d %~dp0..\4.Analizador cripto\backend && .venv\Scripts\activate && python -m uvicorn main:app --reload --port 10000"
...
if %counter% geq %max_wait% (
    ...
    echo         4.Analizador cripto\backend\.venv
    echo.
    pause
    exit /b 1
)
