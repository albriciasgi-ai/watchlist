...
class SupportResistanceIndicator extends IndicatorBase {
  ...
  async fetchData() {
    try {
      ...
      const cacheParams = { days: this.days };
      const cached = await IndicatorCache.get('sr', this.symbol, this.interval, cacheParams);
      if (cached) {
        this.resistances = cached.resistances || [];
        this.supports = cached.supports || [];
        this.consolidationZones = cached.consolidationZones || [];
        this.currentPrice = cached.currentPrice || 0;
        console.log(`[${this.symbol}] ✅ S/R from cache: ${this.resistances.length}R, ${this.supports.length}S`);
      ...
      if (result.success && result.data) {
        ...
        // Guardar en cache
        IndicatorCache.set('sr', this.symbol, this.interval, result.data, cacheParams);
