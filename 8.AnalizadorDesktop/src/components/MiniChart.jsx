...
const MiniChart = ({ symbol, interval, days, indicatorStates, vpConfig, vpFixedRange, oiMode, externalIndicatorManager = null, externalDrawingManager = null, externalDrawingMode = false, onDrawingModeChange = null, onOpenVpSettings, onOpenRangeDetectionSettings, onOpenRejectionPatternSettings, onOpenSupportResistanceSettings, onOpenSR2Settings, onOpenVWAPSettings, onOpenFibonacciSettings, onOpenContinuationPatternSettings, onOpenDoubleTopBottomSettings, onOpenSwingDetectorSettings, rejectionPatternConfig, onFullscreenChange, isFullscreenChild = false, onDrawingsChanged = null, onChartLoaded = null }) => {
  ...
  // 🔄 FIX: Detectar cuando el tab vuelve a estar activo después de inactividad
  // Esto soluciona el gap visual cuando el navegador pausa JavaScript
  const lastVisibilityTimeRef = useRef(Date.now());
  const loadHistoricalDataRef = useRef(null); // Ref para la función loadHistoricalData

  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        const now = Date.now();
        const inactiveTime = now - lastVisibilityTimeRef.current;

        // Si estuvo inactivo más de 30 segundos, recargar histórico para rellenar gaps
        if (inactiveTime > 30000) {
          log.info(`[${symbol}] 🔄 Tab activo después de ${Math.round(inactiveTime/1000)}s de inactividad - recargando histórico`);

          // Recargar datos históricos usando la ref
          if (loadHistoricalDataRef.current) {
            loadHistoricalDataRef.current();
          }

          // Refrescar indicadores
          if (indicatorManagerRef.current && indicatorManagerRef.current.refresh) {
            indicatorManagerRef.current.refresh();
          }
        }
      } else {
        // Tab se vuelve oculto - guardar timestamp
  ...
  // 🔄 GLOBAL: Atajo Ctrl+Shift+R para forzar recarga (limpiar cache y resetear zoom)
  useEffect(() => {
    const handleForceReloadShortcut = (e) => {
      if (e.ctrlKey && e.shiftKey && e.key === 'R') {
        e.preventDefault();
        console.log(`[${symbol}] 🔄 Ctrl+Shift+R detectado - forzando recarga`);
        forceReload();
      }
    };

    window.addEventListener('keydown', handleForceReloadShortcut);
    return () => window.removeEventListener('keydown', handleForceReloadShortcut);
  }, [symbol, interval]); // Re-registrar cuando cambia símbolo/intervalo
  ...
  const viewStateRef = useRef({ offset: 0, zoom: 1, verticalOffset: 0, zoomAutoFixed: false, userZoomed: false });
  ...
  const drawChart = (candles, livePrice = null, mouseX = null, mouseY = null) => {
    ...
    // O si mostramos menos del 20% de las velas disponibles
    ...
    if (showingTooFew && chartWidth > 100 && !viewStateRef.current.userZoomed) {
      ...
      if (Math.abs(newZoom - viewStateRef.current.zoom) > 0.05) {
        ...
        console.log(`[${symbol}] 🔧 Auto-corregido zoom: ${oldZoom.toFixed(3)} -> ${newZoom.toFixed(3)} (mostraba ${preliminaryCandlesPerScreen} velas, ahora ~${Math.floor(targetCandles)} de ${displayCandles.length})`);
      }
    }
  ...
  const loadHistoricalData = async () => {
    try {
      ...
      if (!json.success) {
        console.error(`[${symbol}] ❌ Error en respuesta del servidor:`, json.error || 'Unknown error');
      } else if (!json.data || json.data.length === 0) {
        console.warn(`[${symbol}] ⚠️ Servidor retornó 0 velas nuevas (normal si datos están al día)`);
      }
      ...
      if (json.success && json.data) {
        ...
        if (shouldResetZoom) {
          ...
          // Resetear flag de zoom manual para permitir auto-ajuste
          viewStateRef.current.userZoomed = false;
          ...
          if (canvasRef.current) {
            const rect = canvasRef.current.getBoundingClientRect();
            let chartWidth = rect.width - 75; // Restar márgenes

            // 🔧 FIX: Si el canvas aun no tiene dimensiones validas, usar un valor por defecto
            // Esto puede pasar en Electron cuando el canvas no esta completamente renderizado
            if (chartWidth <= 0) {
              chartWidth = 800; // Valor por defecto razonable
              log.warn(`[${symbol}] ⚠️ Canvas sin dimensiones validas (${rect.width}px), usando chartWidth=${chartWidth}`);
            }
        ...
        // 🔄 FIX: Si la respuesta del servidor falla pero tenemos cache, usarlo
        if (cached && cached.candles && cached.candles.length > 0) {
          console.warn(`[${symbol}] ⚠️ Usando cache existente (${cached.candles.length} velas) porque servidor no retornó datos`);
          candlesRef.current = cached.candles;
          setIsInitialized(true);
          drawChart(candlesRef.current, lastPriceRef.current, mousePos?.x, mousePos?.y);
        } else {
          console.error(`[${symbol}] ❌ Error en respuesta histórica y no hay cache disponible`, json);
        }
      }
    } catch (err) {
      console.error(`[${symbol}] Error cargando histórico`, err);
      // 🔄 FIX: Si hay error de red pero tenemos cache, usarlo
      const fallbackCache = await CandleCache.get(symbol, interval);
      if (fallbackCache && fallbackCache.candles && fallbackCache.candles.length > 0) {
        console.warn(`[${symbol}] ⚠️ Usando cache de fallback (${fallbackCache.candles.length} velas) después de error de red`);
        candlesRef.current = fallbackCache.candles;
        setIsInitialized(true);
        drawChart(candlesRef.current, lastPriceRef.current, mousePos?.x, mousePos?.y);
      }
    }
  ...
  // 🔄 FORCE RELOAD: Limpiar cache y recargar todo (Ctrl+Shift+R o doble-click en info de velas)
  const forceReload = async () => {
    console.log(`[${symbol}] 🔄 FORCE RELOAD: Limpiando cache y reseteando zoom`);

    // 1. Limpiar cache de este símbolo/intervalo
    await CandleCache.clear(symbol, interval);

    // 2. Resetear zoom a default y marcar para auto-ajuste
    viewStateRef.current.zoom = 1;
    viewStateRef.current.userZoomed = false;
    viewStateRef.current.offset = 0;

    // 3. Resetear escala de precios
    priceScaleRef.current.minPrice = null;
    priceScaleRef.current.maxPrice = null;

    // 4. Limpiar velas actuales
    candlesRef.current = [];

    // 5. Recargar datos históricos
    await loadHistoricalData();

    console.log(`[${symbol}] 🔄 FORCE RELOAD completado - ${candlesRef.current.length} velas cargadas`);
  };
    ...
    if (data.topic && data.topic.startsWith("kline.")) {
      ...
      if (klineData && klineData.length > 0) {
        ...
        if (!currentInProgress) {
          ...
          // ⚠️ FIX: Solo guardar si hay suficientes velas (evita sobrescribir cache durante carga inicial)
    ...
    return () => {
      ...
      if (!externalIndicatorManager) {
        ...
        if (indicatorManagerRef.current) {
          ...
          indicatorManagerRef.current = null; // 🛑 Limpiar referencia para evitar uso después de destruir
        }
        ...
        candlesRef.current = [];
        viewStateRef.current.zoomAutoFixed = false; // Resetear flag para proxima carga
  ...
  useEffect(() => {
    ...
    if (isVisible && !wsSubscribedRef.current) {
      ...
      // 🛑 FIX: Detener polling de indicadores para evitar instancias duplicadas
      if (indicatorManagerRef.current && indicatorManagerRef.current.stopAllPolling) {
        indicatorManagerRef.current.stopAllPolling();
      }
      ...
      <div className="mini-chart" ref={containerRef}>
        ...
        <div style={{
          ...
          {indicatorStates && indicatorStates["S&R v2"] && (
            <button
              className="sr2-settings-btn"
              onClick={() => onOpenSR2Settings && onOpenSR2Settings(indicatorManagerRef.current)}
              title="Configurar S&R v2"
              style={{
                background: '#9C27B0',
                color: 'white',
                border: 'none',
        ...
        <div className="fullscreen-modal" onClick={() => setIsFullscreen(false)}>
          <div className="fullscreen-content" onClick={(e) => e.stopPropagation()}>
            ...
            <MiniChart
              ...
              onOpenSR2Settings={onOpenSR2Settings}
