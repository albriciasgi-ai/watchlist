...
function Dashboard({ onViewTrade }) {
  ...
  const fetchDashboardData = async (showLoading = false) => {
    try {
      ...
      try {
        const metricsRes = await fetch(`${API_BASE_URL}/api/metrics?period=all`)
        if (metricsRes.ok) {
      ...
      try {
        const tradesRes = await fetch(`${API_BASE_URL}/api/entries?limit=5`)
        if (tradesRes.ok) {
      ...
      try {
        const equityRes = await fetch(`${API_BASE_URL}/api/metrics/equity-curve`)
        if (equityRes.ok) {
  ...
  if (error) {
    return (
      <div className="empty-state">
        ...
        <p>{error}</p>
        <button className="btn btn-primary" onClick={() => fetchDashboardData(true)}>
          Reintentar
        </button>
      </div>
    )
    ...
    <div className="dashboard">
      <div className="dashboard-row stats">
        ...
        <div className="metric-card">
          ...
          <span className="subtext">Ratio ganancia/perdida</span>
        </div>
        ...
        <div className="metric-card">
          ...
          <span className="subtext">Caida maxima</span>
        </div>
      ...
      <div className="dashboard-row tables">
        <div className="recent-trades">
          ...
          {recentTrades.length > 0 ? (
            ...
            <div className="empty-state">
              <p>No hay trades registrados aun</p>
            </div>
          )}
        </div>
      </div>
