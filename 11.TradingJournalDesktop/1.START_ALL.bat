@echo off
chcp 65001 >nul
title Trading Journal Desktop - Iniciando...

echo ============================================
echo   TRADING JOURNAL DESKTOP - INICIO COMPLETO
echo ============================================
echo.

:: Configuracion
set BACKEND_DIR=..\6.Trading_Journal\backend
set BACKEND_PORT=12000
...
curl -s http://localhost:%BACKEND_PORT%/api/monitor/status >nul 2>&1
if %ERRORLEVEL% EQU 0 (
...
start "Trading Journal Backend" cmd /c "cd /d %BACKEND_DIR% && call .venv\Scripts\activate.bat && python main.py"
...
start "Trading Journal Desktop" cmd /c "npm run dev:electron"
...
echo - Frontend: Electron (puerto 12002 en dev)
echo.
