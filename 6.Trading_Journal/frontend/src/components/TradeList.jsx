...
function TradeList({ onViewTrade }) {
  ...
  useEffect(() => {
    fetchTrades(true)  // Primera carga con spinner

    // Polling cada 5 segundos para actualización en tiempo real
    const interval = setInterval(() => {
      fetchTrades(false)  // Sin spinner durante polling
    }, 5000)
  ...
  const fetchTrades = async (showLoading = false) => {
    try {
      ...
      // La API devuelve {entries: [...], total, limit, offset}
            ...
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort('symbol')} className="sortable">
                  Símbolo <SortIcon column="symbol" />
                </th>
                <th>Dirección</th>
                <th>Estado</th>
                <th>Fuente</th>
